...
function classifyMarket(m) {
  ...
  if (/first.?5|f5|five innings|5 innings/.test(blob)) return 'f5';
  if (/\d(st|nd|rd|th)\s*inning|inning \d/.test(q)) return 'innings';
  // Player props
  if (/to score|points scored|pass yards|rush yards|receiving|strikeouts|hits|home runs|rebounds|assists|touchdown|anytime td|player/.test(q)) return 'player_prop';
  // Team props
  if (/team total|team runs|team points|first team|both teams|to win by/.test(q)) return 'team_prop';
  // Game props
  if (/first score|safety|overtime|exact score|winning margin|race to/.test(q)) return 'game_prop';
  if (t.includes('spread') || /spread|cover|run line|handicap/.test(q)) return 'spread';
  if (t.includes('total') || /\bover\b|\bunder\b|o\/u|more than|less than|total runs|total points|total goals/.test(q)) return 'total';
  if (t.includes('moneyline') || t.includes('winner') || /who will win|moneyline|match winner/.test(q)) return 'moneyline';
  if (t.includes('prop') || /inning|strikeout|hits|bases|period|map |series/.test(q)) return 'prop';
  return 'prop';
}
...
function matchesType(type, marketType) {
  ...
  if (marketType === 'game_lines') return ['moneyline', 'spread', 'total'].includes(type);
  if (marketType === 'player_props') return type === 'player_prop' || type === 'prop';
  if (marketType === 'team_props') return type === 'team_prop';
  if (marketType === 'game_props') return type === 'game_prop';
  if (marketType === 'innings') return ['f5', 'nrfi', 'yrfi', 'innings'].includes(type);
  if (marketType === 'prop') return ['prop', 'player_prop', 'team_prop', 'game_prop', 'f5', 'nrfi', 'yrfi', 'innings'].includes(type);
