...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!r.ok) {
      // Worker may still have tape under a similar slug
      if (tape && tape.hasTape) {
        return res.status(200).json({
          ...
          tags: tape.tags || [],
          tape,
          note: 'Book slug not found on gateway; showing worker tape only',
          bookError: data.message || 'book failed',
        });
      }
      return res.status(200).json({
        ok: false,
        ...
        slug,
        tape,
        hint: 'Worker health OK but this slug may not match tracked markets. Open worker /markets for list.',
      });
    }
